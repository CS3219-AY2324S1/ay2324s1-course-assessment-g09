      ...
      while (messageCount < 2) {
        ...
        if (message !== false) {
          ...
          if (messageCount == 0) {
            ...
            messagePairedObject.user1_videoSocket = videoSocket;
          } else if (messageCount == 1) {
            messagePairedObject.user2_ID = userId;
            messagePairedObject.user2_videoSocket = videoSocket;
          }
      ...
      try {
        ...
        if (message !== false) {
          ...
          if (
            ...
            messagePairedObject.ack1 = true;
            matchedUser = messagePairedObject.user2_ID;
            receiverVideoSocket = messagePairedObject.user2_videoSocket;
            // console.log("Current User ID: ", userId, "   Matched User ID: ", matchedUser)
          } else if (
            ...
            messagePairedObject.ack2 = true;
            matchedUser = messagePairedObject.user1_ID;
            receiverVideoSocket = messagePairedObject.user1_videoSocket;
            // console.log("Current User ID: ", userId, "   Matched User ID: ", matchedUser)
          }
          // Acknowledge and remove the original message from the queue
...
async function findMatch(req, res) {
  ...
  try {
    ...
    if (matchedUser != null) {
      res
        .status(200)
        .json({ success: true, matchedUser, room, receiverVideoSocket });
    } else {
