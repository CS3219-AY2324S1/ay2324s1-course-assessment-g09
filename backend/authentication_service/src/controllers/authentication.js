...
authRouter.post('/signup', async (request, response) => {
    try {
        ...
        const result = await axios.post('http://localhost:3002/users/createUser', {
            email: email,
            username: username,
            password: pwHash,
            role: role
        ...
        if (error.response) {
            ...
            return response.status(error.response.status).json({ message: error.response.data.message })
        } else {
